      ...
      if(!list($id_control_version,$id_user_control_version) = mysql_fetch_row($result_auto_admin_control_version)){
		/** fin select auto_admin_control_version***/
			if($id!=""){
				...
				$respaldo = mysql_to_json($nom_tabla,"and $pk_campo=$id");
				
				
			}
			//echo "$nom_tabla and $pk_campo=$id";
			
			//$condicion= "where $pk_campo=$id";
			$date = date('Y-m-d H:i:s');
			$qry_insert="INSERT INTO auto_admin_control_version(id_control_version,id_usuario,id_auto_admin,id_registro,sql_sess,fecha,abierto,respaldo)
			values (null,'$id_usuario','$id_auto_admin','$id','$sql_sess','$date',1,'$respaldo')";
