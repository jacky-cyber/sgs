...
$nom_campo_tabla = $_POST['nom_campo_tabla'];
     ...
     $result= cms_query($query)or die ("ERROR $php  1 formulario_tabla.php linea 115<br>$query");
      if (list($nom_tabla) = mysql_fetch_row($result)){
			...
			$query= "SELECT campo  
				   ...
				   order by id_campo";
             $result33= cms_query($query)or die ("ERROR $php  1 formulario_tabla.php linea 128<br>$query");
             list($pk_campo) = mysql_fetch_row($result33);
 ...
 $sql = "SELECT * FROM $nom_tabla
  ...
  $qry = cms_query($sql);
   ...
   //echo $query;
          $result44= cms_query($query)or die ("ERROR $php  Linea 70  formulario_tabla.php linea 146<br>$query");
          list($formu) = mysql_fetch_row($result44);
 ...
 for ($i = 0; $i < $num_filas; $i++){			//el num_filas cuenta la cantidad de campos que tiene una tabla 
	...
	if($id!=""){
		...
		if($id_auto_admin!=""){
			//echo "$html_form,$nom_campo,$id_tipo_campo,$id_auto_admin ,$valor_nom_campo,$DATABASE,$id,$id_r<br><br><br>";
			$html_form= html_form2($html_form,$nom_campo,$id_tipo_campo,$id_auto_admin,$valor_nom_campo,$DATABASE,$id,$id_r);		
		
		
		}
	//echo "hola";
	
  //echo "$html_form,$nom_campo,$id_tipo_campo,$id_auto_admin,$valor_nom_campo,$DATABASE,$id,$id_r <br>";
...
if($tabla != $tabla_mandatoria){
	 	...
	 	$registros_form .="<input type=\"hidden\" name=\"$nom_campo\" value=\"$id\">";
...
$contenido =  "
            ...
            $registros_form
                ...
                <tr>
                         ...
                         <input type=\"submit\" name=\"actualizar\" value=\"Agregar...\"></td>
                </tr>
                <tr>
                         <td align=\"center\" class=\"textos\">&nbsp;</td>
                         <td align=\"center\" class=\"textos\">&nbsp;</td>                        
