...
$query= "SELECT id_grupo,grupo 
 		      ...
 		      $result2= cms_query($query)or die (error($query,mysql_error(),$php));
 		       while (list($id_grupox,$grupo) = mysql_fetch_row($result2)){
 		 				if($id_grupox==$id_grupo){
 		 					$lista_sel .="<option value=\"$id_grupox\" selected>$grupo</option>";
 		 				}else{
 		 					$lista_sel .="<option value=\"$id_grupox\" >$grupo</option>";
 		 				}
 		 		 }
...
<input class=\"textos\" type=\"hidden\" name=\"opcion\" value=\"$opcion\">
          ...
          <tr><td align=\"center\" class=\"texnormalbold2\" colspan=\"2\"><h3>Men&uacute; Contenido Cl&aacute;sico	 </h3></td></tr>  
