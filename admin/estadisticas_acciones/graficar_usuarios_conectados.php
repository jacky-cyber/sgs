<?php
	if($_POST["defecto"]){
		...
		$query= "SELECT DATE_FORMAT(fecha,'%d-%m-%y') as fecha, avg(online) AS promedio
	...
	if($_POST["diaConectados"] && $_POST["fecha_inicio"] && $_POST["fecha_termino"]){
	...
	if($_POST["semanaConectados"]){
		...
		$query = "
				SELECT month(fecha) mes,min(DAYOFMONTH(fecha)) minimo, max(DAYOFMONTH(fecha)) maximo, avg(online) promedio
				FROM estadisticas_acciones
	...
	if($_POST["mesConectados"]){
		...
		$result= cms_query($query)or die(error($query,mysql_error(),$php));
		list($fecha_anterior) = mysql_fetch_row($result);
		$query= " SELECT YEAR(fecha) aao, MONTH(fecha) mes, avg(online) promedio
					FROM estadisticas_acciones
					WHERE fecha
