<?php
	...
	if($_POST["defecto"]){
		...
		$query= "SELECT DATE_FORMAT(fecha,'%d-%m-%y') as fecha, COUNT(DISTINCT(ip)) AS cantidad
	...
	if($_POST["diaVisitas"] && $_POST["fecha_inicio"] && $_POST["fecha_termino"]){
	...
	if($_POST["semanaVisitas"]){
		...
		$query = "
				SELECT month(fecha) mes,min(DAYOFMONTH(fecha)) minimo, max(DAYOFMONTH(fecha)) maximo, COUNT(DISTINCT(ip)) AS cantidad
				FROM estadisticas_acciones
	...
	if($_POST["mesVisitas"]){
		...
		$result= cms_query($query)or die(error($query,mysql_error(),$php));
		list($fecha_anterior) = mysql_fetch_row($result);
		$query= " SELECT YEAR(fecha) aao, MONTH(fecha) mes, COUNT(DISTINCT(ip)) AS cantidad
					FROM estadisticas_acciones
					WHERE fecha
