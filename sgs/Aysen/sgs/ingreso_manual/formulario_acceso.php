...
$().ready(function() {
	...
	$(\"#form1\").validate({
		rules: {
			...
			required: function(element) {
				return $('.notificacion:checked').val() == 1;
		...
		messages: {
			...
			email: \"<br>Ingrese su correo electr&oacute;nico\",
...
while (list($id_pais2,$pais) = mysql_fetch_row($result_pais)){
	$paises .= "<option value=\"$id_pais2\" >".utf8_encode($pais)."</option>";
}
...
function desmarca(){
   ...
   $formulario_registro = html_template('formulario_solicitud_papel');	
   
   
   $radios=" <input type=\"radio\" name=\"tipo_folio\" id=\"tipo_folio\" value=\"P\" checked=\"checked\" onclick=\"document.form1.folio.value='';document.form1.folio.disabled = false;ocultaDerivacion();document.form1.fecha_original.value = '#COMODIN#';document.form1.entidad_origen.value = '#COMODIN#';document.form1.radio_oculto.value = this.value;\"/>
        Formulario
          <input type=\"radio\" name=\"tipo_folio\" id=\"tipo_folio\" value=\"C\" onclick=\"document.form1.folio.value='Folio autom&aacute;tico';document.form1.folio.disabled = true;ocultaDerivacion();document.form1.fecha_original.value = '#COMODIN#';document.form1.entidad_origen.value = '#COMODIN#';document.form1.radio_oculto.value = this.value;\" />
Carta
...
$formulario_acceso = cms_replace("#RADIOS#","$radios",$formulario_registro);
...
/***LLENAR ENTIDAD HIJA  se llena con ajax**/
		$filtro_entidad = "<select  class=\"combo\" name=\"id_entidad_hija_origen\"  id=\"id_entidad_hija_origen\"  >$lista_select</select>";
...
$fecha_original = "<input type=\"text\" class=\"combo\" name=\"fecha_original\" id=\"fecha_original\"  readonly=\"readonly\" onclick=\"displayCalendar(document.form1.fecha_original,'dd-mm-yyyy',this)\"/>
					<img src=\"images/calendario.gif\" alt=\"\" border=\"0\" onclick=\"displayCalendar(document.form1.fecha_original,'dd-mm-yyyy',this)\">";
$formulario_acceso = cms_replace("#FECHA_ORIGINAL#","$fecha_original",$formulario_acceso);
$folio_demo=genera_folio($id_entidad,$tipo);

$folio = "<input type=\"text\" class=\"combo\" name=\"folio\" id=\"folio\" maxlength=\"14\" onkeyup=\"ObtenerDatos('index.php?accion=$accion&act=7&fol='+ form1.folio.value +'&radioOculto='+form1.radio_oculto.value+'&axj=1' ,'verifica');\"/>
...
$fecha_ingreso= "<input type=\"text\" class=\"combo\" name=\"fecha_ingreso\" id=\"fecha_ingreso\"  readonly=\"readonly\" onclick=\"displayCalendar(document.form1.fecha_ingreso,'dd-mm-yyyy',this)\"/>
				<img src=\"images/calendario.gif\" alt=\"\" border=\"0\" onclick=\"displayCalendar(document.form1.fecha_ingreso,'dd-mm-yyyy',this)\">";
...
$ciudad = "<input type=\"text\" class=\"combo\" name=\"ciudad\" id=\"ciudad\" />";
  ...
  // $formulario_acceso = $formulario_registro;
   ...
   $fecha = date(d)."-".date(m)."-".date(Y);
	...
	$lista_entidades = "<br><select  class=\"combo\" name=\"id_entidad\" id=\"id_entidad\">$lista_select</select>";
	...
	$notificacion = "<input type=\"radio\" name=\"notificacion\" class=\"notificacion\" id=\"notificacion_1\" $notificacion_1 value=\"1\"  > S&iacute;   
					 <input type=\"radio\" $notificacion_0 name=\"notificacion\" id=\"notificacion_0\" class=\"notificacion\" value=\"0\"  > No    ";
...
";
	...
	$formulario_acceso = cms_replace("#CHECK_OTRA#","$check",$formulario_acceso);
 	  ...
 	  $query= "SELECT id_usuario,nombre , paterno 
               FROM  usuario
			   WHERE id_entidad='$id_entidad_asig' and estado =1
			   order by nombre";
         $result= cms_query($query)or die (error($query,mysql_error(),$php));
