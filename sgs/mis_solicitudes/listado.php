	...
	*/
  	...
  	$sQuery = "
		...
		WHERE 1 
			...
			and id_sub_estado_solicitud not in ($Estados_pendiente_rectificacion)  ".$and."
		$sWhere
		$sOrder
		$sLimit
	";
	...
	$sQuery = "
		...
		FROM   $sTable
		WHERE 1 and id_usuario= $id_user 
...
//		echo "--".count($aColumnsBD)." rrr<br>";
		...
		for ( $i=0 ; $i<count($aColumns) ; $i++ )
		{
			...
			switch ($i) {
                 ...
                 case 5:
				 		...
				 		$id_sub_estado_solicitud =$aRow[$aColumnsBD[4]];
				 		$query= "SELECT estado_para_usuario  
		...
		$folio =$aRow[$aColumnsBD[0]];
		$aRow[6]="<a href=\"?accion=$accion&act=1&folio=$folio\">VER DETALLE</a>";
		$row[] = $aRow[6];
