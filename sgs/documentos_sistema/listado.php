<?php
	...
	$query_perfil="SELECT funcionario 
					   FROM usuario_perfil	
					   WHERE id_perfil='$id_perfil'
					   ";
				
		$result_perfil= cms_query($query_perfil)or die (error($query_perfil,mysql_error(),$php));
		list($id_funcionario) = mysql_fetch_row($result_perfil);
	
	if($id_funcionario==1)
		$lista = html_template('contenedor_listado_historial_respuestas');
	else	
		$lista = html_template('listado_docto_ciudadano');
		...
		while(list($idsol,$archivo_solicitudes_doc,$tipo_documento,$nombre,$apellido,$fecha,$cantidad_descargas) = mysql_fetch_row($result_select_archivos)){
			...
			$link_descarga = "sgs/documentos_sistema/docs/$folio/$idsol/";
				...
				$lista_ficha.="<tr>
							...
							<td class=\"descarga\"><a  onmouseover=\"this.style.cursor='pointer'\" onclick='descarga_archivo(\"index.php?accion=$accion&act=8&iddoc=$idsol&folio=$folio&axj=1\");' >Descargar aqu&iacute;</a></td>
						</tr>
				";
