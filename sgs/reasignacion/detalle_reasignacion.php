        	...
        	*/
				...
				if(!list($id_solicitud_asignacion,$id_estado_asignacion) = mysql_fetch_row($result)){}
						...
						$boton_rechazada = "
											...
											<input type=\"button\" class=\"botonred\" name=\"rechaza_solicitud\" value=\"Rechazar Reasignaci&oacute;n\" onClick=\"javascript: confirmar();\">
											";
						$sin_observacion = 1;
				...
				$lista_obs_solicitud .="<table width=\"100%\"  border=\"0\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\">
					<tr>
					
					<td class=\"alt\">Fecha solicitud</td>
					<td><strong> $fecha_solicita $hora_solicita</strong></td>        	
					</tr>
					<tr>
					<td align=\"left\" class=\"alt\" colspan=\"2\">Observaci&oacute;n de Solicitud:</td>        	
					</tr>
					<tr>
				...
				if($sin_observacion == 1){
					$lista_obs_solicitud .="
					<tr>
					<td align=\"left\" class=\"alt\" colspan=\"2\">Observaci&oacute;n :</td>        	
					</tr>
        	...
        	<td align=\"left\" class=\"alt\">Responsable reasignaci&oacute;n:<strong> $nombre_solicitante</strong></td>
        	<td align=\"right\" class=\"alt\">Fecha soliciutd :<strong> $fecha_solicita $hora_solicita</strong></td>        	
        	</tr>
 ...
 <script language=\"JavaScript\">
     ...
     function confirmar() {  
      	     if (confirm(\"\u00BFEst\u00E1 seguro de rechazar esta solicitud?\")) {     
					...
					alert(\"Debe ingresar un texto explicativo en el campo Observaci\u00F3n.\");
...
$tabla_solicitud_reasignacion ="
 ...
 $query= "SELECT id_solicitud_acceso,
				...
				a.id_entidad,
				a.id_entidad_padre,
				identificacion_documentos,notificacion,id_forma_recepcion,oficina,id_formato_entrega,fecha_inicio,fecha_termino,a.orden,id_estado_solicitud,id_sub_estado_solicitud,id_responsable,fecha_formulacion,id_digitador,hash,id_usuario,prorroga,firmada,id_tipo_solicitud,observacion_adicional
				FROM sgs_solicitud_acceso a
				WHERE folio='$folio' ";
				
				$result= cms_query($query)or die (error($query,mysql_error(),$php));
				
				if(list($id_solicitud_acceso,$folio,$id_entidad,$id_entidad_padre,$identificacion_documentos,$notificacion,$id_forma_recepcion,$oficina,$id_formato_entrega,$fecha_inicio,$fecha_termino,$orden,$id_estado_solicitud,$id_sub_estado_solicitud,$id_responsable,$fecha_formulacion,$id_digitador,$hash,$id_usuario,$prorroga,$firmada,$id_tipo_solicitud,$observacion_adicional) = mysql_fetch_row($result)){
  ...
  //echo $query;
	       	   $query= "SELECT id_solicitud_acceso 
					...
					$boton_asignar_otro_funcionario="<input type='button' name='reasignar_p' id='reasignar_p' value='Asignar a otro funcionario' onClick=\"javascript: confirmar_asignar();\"> 
					 ...
					 $boton_asignar_otro_funcionario = "
           <table width=\"100%\"  border=\"0\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\" >
              <td>Asignar a:</td>
...
";
	...
	$accion_form = "index.php?accion=$accion&act=4";
	 ...
	 $contenido = cms_replace("#MENSAJE#",$boton_asignar_otro_funcionario,$contenido);
	 $contenido = cms_replace("#OTROS#",$tabla_solicitud_reasignacion,$contenido);
