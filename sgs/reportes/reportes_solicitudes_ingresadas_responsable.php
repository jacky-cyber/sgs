...
// aca van las caracteristicas de las solicitudes incluidas las consultas
	...
	if ($id_responsable !=""){
		$condicion = $condicion. " and a.id_responsable = $id_responsable ";
	}
	if ($periodo!=""){
	...
	switch ($nivel) {
		 case "Servicio":
				 $titulo_informe = "Ingresos Totales por responsable seg&uacute;n per&iacute;odo (Resumen por Servicio)";
		 ...
		 case "Entidad":
				...
				$titulo_informe = "Ingresos Totales por responsable seg&uacute;n per&iacute;odo (Resumen por Entidad)";
				...
				$adicional = cms_replace("#COLSPAN#"," colspan=\"3\" ",$adicional); 
				$adicional2 = html_template('encabezado_reporte_nivel_entidad_adicional');	
				$adicional2 = cms_replace("#COLSPAN#"," colspan=\"3\" ",$adicional2); 
				
				$adicional = cms_replace("#COMBO_ADICIONAL#","Estados",$adicional);
				$adicional = cms_replace("#FILTRO_ADICIONAL#","$filtro_estados",$adicional);
				
				$adicional2 = cms_replace("#COMBO_ADICIONAL#","Responsable",$adicional2);
				$adicional2 = cms_replace("#FILTRO_ADICIONAL#","$filtro_responsable",$adicional2);
				...
				$_SESSION['encabezados_mail'] = $encabezado_servicio.$encabezado_entidad.$encabezado_estado.$encabezado_periodo.$encabezado_responsable.$encabezado_categoria.$encabezado_pais.$encabezado_region.$encabezado_comuna;
				$_SESSION['encabezados_csv'] = acentos_inverso($csv_encabezado_servicio.$csv_encabezado_entidad.$csv_encabezado_estado.$csv_encabezado_periodo.$csv_encabezado_responsable.$csv_encabezado_categoria.$csv_encabezado_pais.$csv_encabezado_region.$csv_encabezado_comuna);

				//colocar el responsable
				//$encabezado = cms_replace("#FILTROS_EXTRAS#","$adicional",$encabezado); 
			 ...
			 break;
		case "Solicitud":
		 	$titulo_informe = "Solicitudes por responsable seg&uacute;n per&iacute;odo (Detalle)";
			//encabezados	
			...
			$_SESSION['encabezados_mail'] = $encabezado_servicio.$encabezado_entidad.$encabezado_periodo.$encabezado_mes.$encabezado_estado.$encabezado_responsable.$encabezado_categoria.$encabezado_pais.$encabezado_region.$encabezado_comuna;
			$_SESSION['encabezados_csv'] = acentos_inverso($csv_encabezado_servicio.$csv_encabezado_entidad.$csv_encabezado_periodo.$csv_encabezado_mes.$csv_encabezado_estado.$csv_encabezado_responsable.$csv_encabezado_categoria.$csv_encabezado_pais.$csv_encabezado_region.$csv_encabezado_comuna);
			...
			if($cant_pag > 0){
				...
				$link = "<a href=\"index.php?accion=$accion&act=$act&p={P}&nivel=$nivel&id_entidad_padre=$id_entidad_padre&id_entidad=$id_entidad&periodo=$periodo&mes=$mes&id_estado_solicitud=$id_estado_solicitud&id_responsable=$id_responsable\">"; //Link que queremos ocupar en nuestro paginador
		...
		case "Detalle":
			...
			$titulo_informe = htmlentities("Detalle de la solicitud ");
			//encabezados	
			...
			//procesar consulta busqueda
			 $query= "SELECT id_solicitud_acceso,
						   ...
						   id_sub_estado_solicitud, 
						   id_responsable , 
						   ifnull(c.estado_solicitud,'') estado_solicitud,firmada FROM  sgs_solicitud_acceso a, 
