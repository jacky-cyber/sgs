...
// aca van las caracteristicas de las solicitudes incluidas las consultas
	...
	if ($id_estado_solicitud !=""){
		if ($id_estado_solicitud=="1" or $id_estado_solicitud=="2"){
			$condicion = $condicion. " and a.id_sub_estado_solicitud in  (1,2)";
		}else{
	...
	switch ($nivel) {
		 case "Servicio":
		 		include "estadistica_agregada.php";
				...
				//$result_cantidad= mysql_query($query)or die ("error en la consulta:".mysql_error());											
		 ...
		 case "Entidad":
				...
				$titulo_informe = "Ingresos Totales seg&uacute;n per&iacute;odo (Resumen por Entidad)";
			...
			$_SESSION['encabezados_csv'] = acentos_inverso($csv_encabezado_servicio.$csv_encabezado_entidad.$csv_encabezado_estado.$csv_encabezado_periodo.$csv_encabezado_categoria.$csv_encabezado_pais.$csv_encabezado_region.$csv_encabezado_comuna);
			 ...
			 break;
		case "Solicitud":
		 	$titulo_informe = "Solicitudes seg&uacute;n per&iacute;odo (Detalle)";
			//encabezados	
		...
		case "Detalle":
			...
			/*$query= "SELECT id_solicitud_acceso,
							...
							a.id_estado_solicitud, 
							 b.estado_solicitud estado_padre, 
						   id_sub_estado_solicitud, 
						   id_responsable , ifnull(c.estado_solicitud,'') estado_solicitud,firmada FROM  sgs_solicitud_acceso a, 
